...
func Test_listTargetTask(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args{
				[]config.Target{
					{Url: "http://localhost/", Variables: []config.Variable{{Name: "var4", Value: "special_host_var"}}},
				},
				&config.Task{Command: "command?var1={var1}&var4={var4}", Check: "check?var2={var2}&var3={var3}"},
				&[]config.Variable{
					{Name: "var1", Value: "123 456"},
					{Name: "var2", Value: "[val]"},
					{Name: "var3", Value: "/val"},
					{Name: "var4", Value: "vvvv"},
				},
			},
			[]TargetTask{
				{
					"http://localhost/command?var1=123 456&var4=special_host_var",
		...
		},
		{
			...
			[]TargetTask{
				{
					"http://localhost1/command?token=123",
					"http://localhost1/check?token=123",
				},
				{
					"http://localhost2/command?token=123",
					"http://localhost2/check?token=123",
				},
			},
		},
	}
